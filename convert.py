...
def convert(xml_bytes):
    ...
    for offer in offers:
        ...
        availability = str(offer.get("available", "false")).lower() == "true"
        price = number(text(offer, "price"), 0)
        old_price_raw = text(offer, "price_old")
        old_price = number(old_price_raw, 0) if old_price_raw is not None else None
        stock = number(text(offer, "stock_quantity"), 0)
...
def main():
    ...
    print(f"Generated {OUTPUT_FILE}: {result['total']} offers")
